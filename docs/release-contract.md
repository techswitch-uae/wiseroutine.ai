...
| TRUST-02 | Busy times only erases details, survives reload, and prevents later syncs from restoring them. | `e2e/session-integrity.spec.ts`, `e2e/calendar-repair.spec.ts`; API integrity/privacy tests |
...
| NATIVE-01 | Closing the window does not trap the user: Show Wise Routine is always in the tray menu, independent of Start eligibility. Tray Start expires at scheduled end. | Rust compilation/clock tests; **actual Show/hide/restore and notification behavior require packaged OS acceptance** |
