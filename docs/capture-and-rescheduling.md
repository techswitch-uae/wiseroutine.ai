...
**Postpone / change time** is available on the selected-slot panel, Up Next, unscheduled rows, todo details and guided-session chrome. It supports a custom appointment, 30 minutes later, tomorrow, next week, or returning work to Inbox. Times use the account's zone; missing DST times are rejected and repeated times have an occurrence choice.
...
- Planned/live/bucketed work moves in place. Postponing started/skipped/missed work creates a new appointment and retains the old history. The current todo keeps its notes, links, files and activity identity; late actions on an old appointment cannot finish or reopen its newer appointment.
