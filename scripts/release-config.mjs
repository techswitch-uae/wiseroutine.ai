...
export function releaseConfig(env, config) {
  ...
  if (url.protocol !== "https:" || url.username || url.password || url.pathname !== "/" || url.search || url.hash || !allowed.includes(url.origin)) {
    throw new Error("VITE_API_URL must be an explicit HTTPS API origin allowed by the desktop CSP");
  }
  const key = env.TAURI_SIGNING_PUBLIC_KEY?.trim();
  if (!key || !env.TAURI_SIGNING_PRIVATE_KEY?.trim()) throw new Error("Updater public and private signing keys are required");
  // Tauri's public key is a base64-encoded minisign public-key document.
  const document = Buffer.from(key, "base64").toString("utf8");
  if (!document.startsWith("untrusted comment:") || !document.includes("\n")) throw new Error("Invalid updater public-key document");
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (!process.argv.includes("--check")) writeFileSync(resolve(root, "apps/desktop/src-tauri/tauri.release.generated.json"), `${JSON.stringify(overlay, null, 2)}\n`);
}
