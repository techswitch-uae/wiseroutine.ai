...
const addons = readdirSync(resolve(root, "addons")).map((name) => {
  ...
  if (!/^[a-z0-9]+([.-][a-z0-9]+)*$/.test(manifest.id)) throw new Error(`Unsafe addon id: ${manifest.id}`);
  for (const file of ["addon.js", "manifest.json"]) {
    if (!existsSync(resolve(dir, "dist", file))) throw new Error(`Missing ${name}/dist/${file}. Build through the root Turbo graph, not Vite alone.`);
  }
...
for (const addon of addons) cpSync(resolve(addon.dir, "dist"), resolve(target, addon.id), { recursive: true });
