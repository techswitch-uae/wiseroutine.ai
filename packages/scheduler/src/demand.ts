...
export function sessionsNeededToday(
  ...
  if (!Number.isFinite(activity.sessionMinutes) || activity.sessionMinutes <= 0 ||
      !Number.isFinite(minimum.value) || minimum.value < 0) {
    throw new RangeError("Activity duration and minimum must be finite, with a positive duration");
  }
