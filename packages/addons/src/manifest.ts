...
const safeSettingKey = (key: string): boolean =>
    ...
    case "open:external": {
      const allowed = new Set(
        held
          .filter((c) => {
            if (
              request.kind !== "net:fetch" ||
              c.kind !== "net:fetch" ||
              !request.auth
            )
              return true;
            return (
              c.auth?.secret === request.auth.secret &&
              c.auth.header.toLowerCase() ===
                request.auth.header.toLowerCase() &&
              (c.auth.prefix ?? "") === (request.auth.prefix ?? "")
            );
          })
...
export function isPlainHttpsOrigin(value: string): boolean {
  if (/[\s;"'<>*]/.test(value)) return false; // CSP source tokens, not just parseable URLs.
  let url: URL;
  try {
    url = new URL(value);
  } catch {
...
export function parseManifest(raw: unknown): AddonManifest | null {
  ...
  try {
    if (JSON.stringify(raw).length > MAX_MANIFEST_BYTES) return null;
  } catch {
    return null;
  }
  if (
    ...
    return null;
  if (typeof m.version !== "string" || !SEMVER.test(m.version)) return null;
...
function parseAuth(raw: unknown): FetchAuth | null {
  ...
  if (typeof a.secret !== "string" || !safeSettingKey(a.secret)) return null;
...
function parseContributions(raw: unknown): AddonContribution[] | null {
  ...
  for (const entry of raw) {
    ...
    if (c.key.length > 64 || seen.has(c.key)) return null;
    seen.add(c.key);
...
function parseActivityTypes(raw: unknown): AddonActivityType[] | null {
  ...
  if (parseContributions(raw) === null) return null;
...
function parseSettings(
  ...
  for (const entry of raw) {
    ...
    if (typeof f.key !== "string" || !safeSettingKey(f.key)) return null;
    ...
    if (f.showWhen !== undefined) {
      ...
      if (typeof s.key !== "string" || !safeSettingKey(s.key)) return null;
