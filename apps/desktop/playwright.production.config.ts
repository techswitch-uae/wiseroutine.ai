...
export default defineConfig({
  ...
  testMatch: [
    ...
    ...contracts.map((name) => `**/e2e/${name}.spec.ts`),
  ],
  testIgnore: ["**/.playwright/**", "**/node_modules/**", "**/.output/**"],
  outputDir: ".playwright/built/test-results",
  reporter: [
    [process.env.CI ? "list" : "line"],
    ["html", { open: "never", outputFolder: ".playwright/built/report" }],
  ],
  webServer: [
    {
      ...worker,
      command: worker.command.replace(
        ".wrangler/e2e-state",
        ".wrangler/e2e-built-state",
      ),
      env: { ...worker.env, WRANGLER_LOG_PATH: ".wrangler/e2e-built-logs" },
    },
    {
      command:
        "pnpm exec turbo run build --filter=@wiseroutine/desktop... && node e2e/serve-built.mjs",
      url: APP_URL,
