...
test.each(["manual", "prompt", "unknown"])(
  "%s never auto-moves or invents an outcome",
  (startPolicy) => {
    for (const now of [
      startsAt - 1,
      startsAt,
...
test.each(["planned", "live"])(
  "guided %s starts only while time remains, including a late sweep",
  (status) => {
...
test.each([
  ...
  "cancelled",
  "bucketed",
])("%s never auto-starts", (status) => {
  expect(graceAction({ ...slot, status, startPolicy: "auto" }, startsAt)).toBe(
    "leave",
  );
});
