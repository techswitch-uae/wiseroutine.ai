...
test.each([false, true])(
  "manual slot stays put after the cutoff, across repeated sweeps and sleep (pinned=%s)",
  async (isLocked) => {
    const { at, slot, job, activityId } = await fixture();
    await userDb().activity.update({
      where: { id: activityId },
      data: { startPolicy: "manual", graceMinutes: 0 },
    });
    await userDb().slot.update({
      where: { id: slot.id },
      data: { isLocked, autoMoveCount: 99 },
    });
    ...
    for (const now of [
      ...
      await sweepGrace(
        ...
        config,
        now,
        resolveFeatures({ guided_sessions: true }),
      );
      expect(
...
test("an earlier auto-start never completes a later manual Resume or starves the next automatic completion", async () => {
  const { at, slot, job } = await fixture();
  await sweepGrace(job, config, at, resolveFeatures({ guided_sessions: true }));
  await setSlotStatus(
    userDb(),
  ...
  expect(await nextGraceDeadline(userDb(), at + 3_000)).toBeUndefined();
  const automatic = await placeSlot(
    userDb(),
    {
  ...
  expect(
    (await autoSlotsToComplete(userDb(), at + 601_000, 1)).map((row) => row.id),
  ).toEqual([automatic.id]);
  await sweepGrace(job, config, at + 601_000);
  expect(
    (await userDb().slot.findUnique({ where: { id: slot.id } }))?.status,
  ).toBe("started");
  expect(
    (await userDb().slot.findUnique({ where: { id: automatic.id } }))?.status,
  ).toBe("completed");
});
...
test("a late guided sweep starts only while time remains and preserves the original end", async () => {
  ...
  expect(
    await userDb().slot.findUnique({ where: { id: slot.id } }),
  ).toMatchObject({ status: "started", endsAt: new Date(at + 600_000) });
