...
export async function applyBillingEvent(
  ...
  return processWebhook(directory, "stripe", event.id, now, async (tx) => {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      subscriptionId =
        typeof source === "string" ? source : (source?.id ?? null);
      if (!subscriptionId && userId)
        subscriptionId =
          (await getSubscription(tx, userId))?.stripeSubscriptionId ?? null;
    } else return;
    ...
    const customerId =
      typeof current.customer === "string"
        ...
        : current.customer.id;
    userId ??=
      current.metadata?.userId ||
      (await findUserByStripeCustomer(tx, customerId));
    if (!userId)
      throw new Error("Subscription delivery has no user mapping yet");
    const previous = await getSubscription(tx, userId);
    if (
      previous?.stripeSubscriptionId &&
      previous.stripeSubscriptionId !== current.id
    ) {
